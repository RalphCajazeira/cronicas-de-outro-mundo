...
import { contentRefSchema } from './content.schemas.js';
...
export function createContentRouter(repository: ContentRepository) {
  ...
  return Router().get('/:contentRef', async (request, response, next) => {
    try { response.json(await service.get(contentRefSchema.parse(request.params.contentRef))); } catch (error) { next(error); }
  });
}
