...
import { actorRefSchema, gameScopeSchema } from './actors.schemas.js';
...
export function createGetActorController(repository: ActorRepository): RequestHandler {
  ...
  return async (request, response, next) => {
    try {
      response.json(await service.get(gameScopeSchema.parse(request.query), actorRefSchema.parse(request.params.actorRef)));
