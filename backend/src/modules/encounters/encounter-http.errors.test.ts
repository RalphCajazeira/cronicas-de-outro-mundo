...
const cases: Array<[EncounterErrorCode, number, string, boolean]> = [
  ...
  ['ENCOUNTER_PARTICIPANT_NOT_READY', 422, 'CHARACTER_NOT_READY', false],
  ...
  ] as EncounterErrorCode[]).map((code): [EncounterErrorCode, number, string, boolean] => [code, 409, 'ENCOUNTER_AUTHORITY_DRIFT', false]),
  ...([
    ...
    'ENCOUNTER_ROLL_INVALID', 'ENCOUNTER_EFFECT_OWNERSHIP_CONFLICT', 'ENCOUNTER_EFFECT_ORIGIN_REQUIRED',
