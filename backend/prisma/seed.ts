...
async function main() {
  const player = await prisma.player.upsert({ where: { slug: 'ralph' }, update: { displayName: 'Ralph' }, create: { slug: 'ralph', displayName: 'Ralph' } });
  const world = await prisma.world.upsert({ where: { playerId_code: { playerId: player.id, code: 'elarion' } }, update: { name: 'Elarion' }, create: { playerId: player.id, code: 'elarion', name: 'Elarion' } });
  const campaign = await prisma.campaign.upsert({ where: { worldId_code: { worldId: world.id, code: 'main-campaign' } }, update: { name: 'Campanha Principal', status: CampaignStatus.ACTIVE }, create: { worldId: world.id, code: 'main-campaign', name: 'Campanha Principal', status: CampaignStatus.ACTIVE } });
  const ralph = await prisma.actor.upsert({
  ...
  const breezeStep = await prisma.contentDefinition.upsert({
