...
O backend também publica o orquestrador persistente `manageEncounter` sobre o núcleo `core-v1-encounter-v1`. Cada resposta define a próxima operação em `nextRequiredAction`; o fluxo granular deixa de ser opcional quando esse campo o exige. `resolve_beat` recebe uma intenção de alto nível com até três componentes, internaliza timeline, reações, ações de aliados/inimigos e conclusão, e persiste um checkpoint atômico, mas só deve ser escolhido sem requisito granular pendente e com lifecycle compatível. O cliente usa `expectedStateVersion` e nunca fornece rolls ou resultados mecânicos. Pela Action, participantes precisam ser atores persistidos na Campaign; participantes efêmeros não são aceitos.
...
Quando `resolve_beat` for a operação aplicável, o plano manual/assistido continua limitado a três componentes e usa seu pipeline autoritativo. Cada componente pode ter uma condição fechada `when`: ator opcional, recurso `hp|mana|sp`, comparação percentual acima/abaixo e percentual 0–100. A condição é avaliada no snapshot do início do beat. Se falsa, o fallback permitido é somente `skip` ou `defend`; sem fallback, equivale a `skip`. Não há repetição, código, expressão arbitrária ou loop livre. Se todas as etapas estiverem condicionadas e nenhuma puder executar, o backend rejeita sem escrita e pede uma etapa incondicional ou fallback.
...
`flee` expressa a intenção final de alcançar `far` ou `out_of_range`, não teleporte. O backend aplica somente o próximo `disengage`/`run` legal, marca staging como modificação e, na estratégia automática `escape`, continua sem nova pergunta até confirmar `out_of_range`, bloqueio, decisão, terminalidade ou budget. Nunca narre fuga concluída antes do delta autoritativo. Chegar a `out_of_range` mantém o participante no encontro no estado atual; não invente remoção, derrota ou morte.
