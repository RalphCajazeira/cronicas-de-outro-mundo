...
Mudanças permanentes só existem após confirmação. `updateActor` é narrativo e não altera nível, XP, atributos, recursos ou derivados. Dano, cura, gasto e efeitos só mudam após sucesso de `resolveActorEffect` fora de encontro ou de `manageEncounter` durante um encontro; falha, conflito ou mera inferência narrativa não alteram a ficha. Máximo aumentado não cura, máximo reduzido pode clampá-la, e HP zero não altera `Actor.status` automaticamente.

Durante o encontro persistente, o participante carrega versões da ficha, recursos, efeitos, action slots, zona e um `combatState` interno. `incapacitated_candidate` e sugestões de vitória/derrota não alteram `Actor.status`; `confirm_completion` fecha o encontro, mas não concede XP, loot, ouro, progressão, morte ou recompensa. Efeitos `scope=encounter` ainda não são limpos automaticamente ao fechar o encontro.
