...
RC1.1 permanece histórica (nível 20/atributo efetivo 30) e RC1.2 preserva a progressão sem cap de gameplay. Campanhas pinadas não mudam silenciosamente. Novos mundos usam RC1.3, que herda a progressão RC1.2 e acrescenta furtividade, detecção, contexto e surpresa versionados.
...
Modificadores secundários vindos de conteúdo, equipamento ou status são allowlisted, inteiros, limitados pelo tier e recompostos no snapshot. Campo omitido vale zero. Equipamento contribui somente enquanto fisicamente equipado; status e buffs/debuffs deixam de contribuir ao expirar/remover. Replay idempotente não reaplica o mesmo bônus.
...
Na criação inicial confirmada, `startGame` publica definições, cria vínculos, concede `initialInventory`, equipa e calcula `readiness` na mesma transação. Na Criação Rápida, `starterBlueprint` deriva a mecânica oficial sem aceitar profile/inventorySpec do cliente. Uma definição reutilizada é consultada e referenciada sem reenviar ficha. Cada conteúdo físico aparece em um único grant agregado, com refs determinísticas; todos os grants precedem equipamento. Só comece a aventura mecânica quando `canBeginMechanically`, `hasUsableOffensiveAction`, `inventoryValid` e `equipmentValid` forem verdadeiros.
