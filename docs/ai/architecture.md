...
Ações comuns usam primitivas já existentes. Movimento/fuga usam `movement`; observar, defender, proteger, interceptar, ajudar, interagir e improvisar consomem oportunidade por `wait/minor`. `wait` tem preparação zero e recuperação canônica de 100 ticks, agenda somente `action_started` e `actor_ready` e não agenda `action_effect` sem profile mecânico. Defesa/proteção/interceptação adicionam capacidades de reação de uso único ao snapshot, válidas somente durante o beat que as preparou; ataques mecânicos permitem essas reações, o resultado público marca `GUARD_PREPARED`, e bloqueio só é narrável quando há reação resolvida. Preparação usa `actionPlans` persistidos com trigger fechado e só executa conteúdo conhecido quando o trigger ocorre. Ataque, magia e item reutilizam o loader autoritativo de conteúdo/inventário; nenhuma ausência de habilidade é substituída silenciosamente por outra ação.

Fuga é intenção final, não uma primitiva de teleporte. Um único derivador no módulo de beats transforma `flee` no próximo movimento legal: `engaged → near` por `disengage`; depois `run` avança no máximo duas zonas até `far` ou `out_of_range`. O mesmo derivador orienta normalização, validação, cápsula e política `escape`. Ao atingir `out_of_range`, a política para sem agendar movimento nulo; o participante permanece `ready` e não gera candidato terminal somente pela zona.
...
A ampliação local de 2026-07-23 mantém essa arquitetura e acrescenta duas entradas ao mesmo `resolve_beat`: plano curto condicionado e política automática. `create assisted` converte grupos semânticos em participantes/relações/zonas canônicos e valida prontidão antes do commit. A `scene` v2 projeta o catálogo em uma leitura de conteúdo e uma de inventário para todos os participantes e expõe custo, alcance, alvos e blockers sem carregar perfil interno. Ações detalhadas ficam com os atores controláveis; NPCs levam contagens, ameaças, estado e perfil tático. O alvo de catálogo detalhado é 256, sem descartar ação utilizável do protagonista; excesso bloqueado ou resumido é marcado como parcial. A cápsula visa 64 KiB no encontro comum e 128 KiB com oito participantes, com rejeição fechada acima do hard cap público de 256 KiB UTF-8. A transação preserva os 30 segundos apenas como limite extremo.

No plano, `when` compara somente percentual de HP/Mana/SP no snapshot inicial; `skip` omite e `defend` aplica a capacidade defensiva autoritativa. Não existe AST, expressão, código ou repetição. Na política, o service seleciona deterministicamente apenas ações projetadas, conserva recursos e chama `executeOneResolvedBeat`, que é a extração do pipeline anterior — não um resolver paralelo. A autoridade e o catálogo inicial são carregados uma vez; cada beat evolui o estado em memória e recompõe localmente apenas disponibilidade/consumo, antes de uma única cápsula final. Uma transação processa seis beats por padrão e no máximo 12, três componentes e até quatro NPCs elegíveis por beat, adiando os demais de forma explícita e determinística. O resumo público limita o timeline a 32 eventos, mas calcula deltas sobre todos os relatórios e grava um único checkpoint externo com rolls completos.

Paradas `technical` (`processing_limit`/budget) usam nova versão e nova chave em outra chamada, sem manter lock entre chamadas e sem pedir decisão. Paradas `decision` preservam alternativas e retornam controle. Versão/authority drift continuam falhando fechados sob os locks existentes; terminal passa pelo finalizador e não cria morte, XP, ouro, loot ou recompensa antecipada. A telemetria registra operação, modo, etapa da cápsula, queries, banco acumulado, beats, ações, parada e commit/erro, nunca payload, narrativa, SQL, rolls, UUIDs ou secrets.
