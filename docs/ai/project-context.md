...
- O primitive `wait` é uma ação temporal sem profile mecânico: ocupa o slot selecionado por 100 ticks, gera somente início e readiness e é resolvido sem `action_effect`. As ações genéricas reutilizam esse caminho; `defend`, `protect` e `intercept` só preparam capabilities de guarda para o beat atual, e `prepare` só armazena seu plano após o `wait` concluir. A correção é local e aguarda revisão pré-commit, sem alterações remotas.
