...
begin
  ...
  if p_character_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
    ...
    if v_character_id is null then
      select source_id into v_character_id
      from public.rpg_actors
      where id=v_ref_uuid and owner_id=v_owner and source_kind='character'
      limit 1;
    end if;
  else
    select id into v_character_id
    from public.rpg_characters
    where owner_id=v_owner and lower(name)=lower(trim(p_character_id))
  ...
  if p_content_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
    ...
    where c.owner_id=v_owner and c.world_id=ca.world_id and c.status='active'
    ...
    order by case when lower(c.code)=lower(trim(p_content_id)) then 0 else 1 end,c.updated_at desc limit 1;
  end if;
  ...
  if p_operation in ('learn','grant','add','equip') then
    ...
    values(v_owner,v_character_id,v_content_id,
      coalesce(nullif(p_changes->>'state',''),case when p_operation='equip' then 'owned' else 'known' end),
      coalesce(nullif(p_changes->>'rank','')::int,1),coalesce(nullif(p_changes->>'progress','')::int,0),
      coalesce(nullif(p_changes->>'mastery','')::int,0),coalesce((p_changes->>'equipped')::boolean,p_operation='equip'),
    ...
    on conflict(character_id,content_id) do update set
      state=coalesce(nullif(p_changes->>'state',''),public.rpg_character_content.state),
