drop function if exists public.rpg_gateway_manage_character_content(text,uuid,text,uuid,jsonb);
...
begin
  ...
  select * into v_char
  from public.rpg_characters
  where id=v_character_id and owner_id=v_owner;
  ...
  if p_operation in ('learn','grant','add','equip') then
    ...
    values(
      v_owner,v_character_id,v_content_id,
